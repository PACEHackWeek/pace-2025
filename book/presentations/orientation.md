# Orientation

```{admonition} Let's Get Started
:class: note
- View slides {{ l1 }}
- Download content {{ d1 }}
- Recording available {{ v1 }}
```

| Title | Slides | Recording |
| :---- | :----: | :-------: |
| Welcome, Introductions, and Housekeeping | [{{ l1 }}][hi-l] | {{ v0 }} |
| Orientation to CryoCloud                 | [{{ l1 }}][cc-l] | {{ v0 }} |
| Group Projects                           | [{{ l1 }}][gp-l] | {{ v0 }} |

```{admonition}
:class: warning

When you get to "Server Options" page after logging in, scroll down to "Bring your own image" option and enter **ghcr.io/nasa/oceandata-notebooks:latest** into the "Custom image" field.

![server_options](../img/server_options.png)
```

[hi-l]: https://docs.google.com/presentation/d/1BZ1t-3GsQ8d6ZeMfittVVJcwt4CPEUOAIeQgCfQTWcs/present
[cc-l]: https://docs.google.com/presentation/d/1GX_Sq3eubuoJVlpz2g4R3qvhCjiWZq6NAuwJU5Yd5mk/present
[gp-l]: https://docs.google.com/presentation/d/1cyGaWCFgtsUjcKRfZhx1RTiwA367pAXy/present

